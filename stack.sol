...
contract StakingContract{
    ...
    uint256 public withdrawInterval = 120;
    uint public id =0;
    ...
    mapping(uint => mapping(address => uint256)) public stakedTime;   // it is mapping id & address with staked time.
    mapping(uint => uint)public amountStaked;  // it is mapping which id stake how much amount of token.
    mapping(address => uint[])public addrToId;   // here address is mapping the id.
    ...
    function stakedDetail(address _stakedAddr)public view returns(uint[] memory){
        return (addrToId[_stakedAddr]);
        // for(uint i=0;i<addrToId[_stakedAddr].length;i++){
        //     return (addrToId[_stakedAddr][i], amountStaked[addrToId[_stakedAddr]][i]);
        // }
    }
